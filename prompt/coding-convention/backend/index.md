# Uma Crown Simulator - バックエンド規約（NestJS / Prisma）

このファイルを読んだ AI はバックエンド固有のフレームワーク規約を把握し、コード修正・レビュー・新規実装に適用してください。
...
9. [Prisma エラー種別ハンドリング](#9-prisma-エラー種別ハンドリング)
10. [HTTP ステータスコードの判断基準](#10-http-ステータスコードの判断基準)
11. [成功レスポンスの形式](#11-成功レスポンスの形式)
12. [@Public() 使用基準](#12-public-使用基準)
13. [ValidationPipe グローバル設定](#13-validationpipe-グローバル設定)
14. [Response DTO（Prisma 型の直接返却禁止）](#14-response-dtoprisma-型の直接返却禁止)
15. [Prisma の select / include 方針](#15-prisma-の-select--include-方針)
16. [N+1クエリ防止](#16-n1クエリ防止)
17. [横断処理の使い分け（Guard / Interceptor / Middleware / Filter）](#17-横断処理の使い分けguard--interceptor--middleware--filter)
18. [Prisma トランザクション](#18-prisma-トランザクション)
19. [Module 登録パターン](#19-module-登録パターン)
...
- **Service**: Prisma エラーをキャッチし `DatabaseException` に変換してから rethrow する（→ §9）
...
## 14. Response DTO（Prisma 型の直接返却禁止）
...
## 17. 横断処理の使い分け（Guard / Interceptor / Middleware / Filter）
