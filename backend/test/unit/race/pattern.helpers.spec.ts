...
describe('getConsecutiveLength', () => {
  ...
  it('隣接なし → 1', () => {
    const grid = new Map<string, RaceRow>([['classic|1|false', makeRace()]]);
    expect(getConsecutiveLength(grid, 'classic|4|false')).toBe(1);
  });
});
...
describe('buildPatternFromGrid', () => {
  ...
  it('配置したスロット以外は空（race_id: null）', () => {
    ...
    const grid = new Map([['classic|5|true', race]]);
    const pattern = buildPatternFromGrid(grid);
    const otherSlots = pattern.classic.filter(
      (s) => !(s.month === 5 && s.half === true),
    );
...
describe('getAllRacesInPattern', () => {
  it('パターン内の race_id に対応するレースを返す', () => {
    ...
    const pattern = buildPatternFromGrid(grid);
    const result = getAllRacesInPattern(pattern, [
      race1,
      race2,
      makeRace({ race_id: 99 }),
    ]);
  ...
  it('allGRaces に存在しない race_id は無視される', () => {
    ...
    expect(
      getAllRacesInPattern(pattern, [makeRace({ race_id: 99 })]),
    ).toHaveLength(0);
  });
});
...
describe('calcBCStrategy', () => {
  it('芝C以上かつ中距離C以上 → null（Bパターン・補修不要）', () => {
    ...
    expect(
      calcBCStrategy(
        bcRace,
        makeUma({ turf_aptitude: 'A', classic_aptitude: 'A' }),
      ),
    ).toBeNull();
  });
  ...
  it('C境界値（芝C・距離C）→ null', () => {
    ...
    expect(
      calcBCStrategy(
        bcRace,
        makeUma({ turf_aptitude: 'C', classic_aptitude: 'C' }),
      ),
    ).toBeNull();
  });
  ...
  it('芝G → {芝: 3}（G→C に3枚必要、上限クリップ）', () => {
    ...
    expect(
      calcBCStrategy(
        bcRace,
        makeUma({ turf_aptitude: 'G', classic_aptitude: 'A' }),
      ),
    ).toEqual({ 芝: 3 });
  });
  ...
  it('距離D → {中距離: 1}（D→C に1枚必要）', () => {
    ...
    expect(
      calcBCStrategy(
        bcRace,
        makeUma({ turf_aptitude: 'A', classic_aptitude: 'D' }),
      ),
    ).toEqual({ 中距離: 1 });
  });
  ...
  it('芝D・距離D → {芝: 1, 中距離: 1}', () => {
    ...
    expect(
      calcBCStrategy(
        bcRace,
        makeUma({ turf_aptitude: 'D', classic_aptitude: 'D' }),
      ),
    ).toEqual({ 芝: 1, 中距離: 1 });
  });
  ...
  it('距離E → {中距離: 2}（E→C に2枚必要）', () => {
    ...
    expect(
      calcBCStrategy(
        bcRace,
        makeUma({ turf_aptitude: 'A', classic_aptitude: 'E' }),
      ),
    ).toEqual({ 中距離: 2 });
  });
  ...
  it('ダートBCレース: ダート適性を参照する', () => {
    ...
    expect(
      calcBCStrategy(
        bcRace,
        makeUma({ dirt_aptitude: 'G', mile_aptitude: 'A' }),
      ),
    ).toEqual({ ダート: 3 });
  });
  ...
  it('短距離BCレース: sprint_aptitude を参照する', () => {
    ...
    expect(
      calcBCStrategy(
        bcRace,
        makeUma({ turf_aptitude: 'A', sprint_aptitude: 'E' }),
      ),
    ).toEqual({ 短距離: 2 });
  });
  ...
  it('長距離BCレース: long_distance_aptitude を参照する', () => {
    ...
    expect(
      calcBCStrategy(
        bcRace,
        makeUma({ turf_aptitude: 'A', long_distance_aptitude: 'F' }),
      ),
    ).toEqual({ 長距離: 3 });
  });
});
...
describe('raceMatchesAptitude', () => {
  ...
  describe('bcFinalRace 未指定（通常モード）', () => {
    it('芝C以上かつ中距離C以上 → true', () => {
      expect(
        raceMatchesAptitude(makeRace({ race_state: 0, distance: 3 }), aptGood),
      ).toBe(true);
    });
    it('芝E（C未満）→ false', () => {
      expect(
        raceMatchesAptitude(makeRace({ race_state: 0, distance: 3 }), aptWeak),
      ).toBe(false);
    });
    it('ダートC以上かつマイルC以上 → true', () => {
      expect(
        raceMatchesAptitude(makeRace({ race_state: 1, distance: 2 }), aptGood),
      ).toBe(true);
    });
    it('C境界値（芝C・距離C）→ true', () => {
      const aptC = {
      ...
      expect(
        raceMatchesAptitude(makeRace({ race_state: 0, distance: 3 }), aptC),
      ).toBe(true);
    });
  });
  ...
  describe('bcFinalRace 指定（BCモード）', () => {
    ...
    it('馬場が異なる → false', () => {
      expect(
        raceMatchesAptitude(
          makeRace({ race_state: 1, distance: 3 }),
          aptGood,
          bcRace,
        ),
      ).toBe(false);
    ...
    it('距離が異なる → false', () => {
      expect(
        raceMatchesAptitude(
          makeRace({ race_state: 0, distance: 2 }),
          aptGood,
          bcRace,
        ),
      ).toBe(false);
...
describe('isRaceRunnable', () => {
  it('芝D・中距離D → true（D=スコア0以上）', () => {
    ...
    expect(isRaceRunnable(makeRace({ race_state: 0, distance: 3 }), apt)).toBe(
      true,
    );
  });
  ...
  it('ダートD・マイルD → true', () => {
    ...
    expect(isRaceRunnable(makeRace({ race_state: 1, distance: 2 }), apt)).toBe(
      true,
    );
  });
  ...
  it('短距離D → true', () => {
    ...
    expect(isRaceRunnable(makeRace({ race_state: 0, distance: 1 }), apt)).toBe(
      true,
    );
  });
});
...
describe('calcRunnableEnhancement', () => {
  ...
  it('ダート短距離レース: ダート適性不足を補修', () => {
    ...
    expect(
      calcRunnableEnhancement(
        makeRace({ race_state: 1, distance: 1 }),
        apt,
        null,
      ),
    ).toEqual({ ダート: 2 });
...
describe('calculateFactorComposition', () => {
  ...
  it('常に6要素を返す（戦略あり）', () => {
    expect(
      calculateFactorComposition(makeUma(), [makeRace()], { 芝: 2 }),
    ).toHaveLength(6);
  });
  ...
  it('isLarc=true: 戦略から芝・中距離が除外される', () => {
    ...
    expect(result.filter((f) => f === 'ダート').length).toBeGreaterThanOrEqual(
      1,
    );
  });
  ...
  it('出力は FACTOR_SORT_ORDER でソートされる（芝 < ダート < 距離 < 自由）', () => {
    const uma = makeUma({ turf_aptitude: 'G', classic_aptitude: 'G' });
    const result = calculateFactorComposition(uma, [
      makeRace({ race_state: 0, distance: 3 }),
    ]);
    // 自由より前には馬場/距離因子が来る
    ...
    if (result[0] !== '自由') {
      ...
      for (let i = 1; i < result.length; i++) {
        expect(order[result[i]] ?? 98).toBeGreaterThanOrEqual(
          order[result[i - 1]] ?? 0,
        );
      }
    }
