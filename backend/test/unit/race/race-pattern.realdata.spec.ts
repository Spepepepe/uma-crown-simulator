...
const LARC_MANDATORY_NAMES = new Set([
  '日本ダービー',
  'ニエル賞',
  '凱旋門賞',
  '宝塚記念',
  ...
  for (const umaName of TARGET_UMAS) {
    describe(umaName, () => {
      ...
      it('パターン生成がエラーなく完了する', async () => {
        ...
        const umaId = TARGET_UMAS.indexOf(umaName) + 1;
        await expect(
          service.getRacePattern(userId, umaId),
      ...
      it('BCパターンにBC最終レースが配置されている', async () => {
        ...
        const bcRaceNames = mockAllGRaces
          .filter((r) => r.bc_flag)
          .map((r) => r.race_name);
        ...
        for (const pattern of bcPatterns) {
          ...
          const hasBC = allSlots.some(
            (s) => s.race_name && bcRaceNames.includes(s.race_name),
          );
