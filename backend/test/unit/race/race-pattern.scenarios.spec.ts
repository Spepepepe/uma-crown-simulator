...
const allRacesFromJson: RaceRow[] = Object.values(raceJson).map(
  (r: any, idx: number) => ({
    race_id: idx + 1,
...
const raceByName = new Map<string, RaceRow>(
  allRacesFromJson.map((r) => [r.race_name, r]),
);
...
const TARGET_UMAS = Object.keys(umaJson);
        ...
        describe(`BC残${bcCount}件`, () => {
          ...
          describe('ラーク残存あり', () => {
            it(`BC${actualBCCount}件 + larcパターンが生成される`, async () => {
              ...
              const bcPatterns = result.patterns.filter(
                (p) => p.scenario === 'bc',
              );
              const larcPatterns = result.patterns.filter(
                (p) => p.scenario === 'larc',
              );
            ...
            it('未出走レースが全件いずれかのパターンに含まれる', async () => {
              ...
              const unrunRaceIds = allGRaces
                .filter(
                  (r) =>
                    !currentRunRaceIds.has(r.race_id) &&
                    isRaceRunnable(r, baseAptState),
                )
                .map((r) => r.race_id);
      ...
      for (const runCount of BC_COUNTS) {
        ...
        it(`ランダム${actualRunCount}件BC出走済みで残レース全件パターン収録`, async () => {
          ...
          const unrunRaceIds = allGRaces
            .filter(
              (r) =>
                !runRaceIds.has(r.race_id) && isRaceRunnable(r, baseAptState),
            )
            .map((r) => r.race_id);
