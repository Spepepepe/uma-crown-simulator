...
describe('SeedService', () => {
   ...
   */
  function mockLoadJsonFile(returnValues: Record<string, unknown>): jest.SpyInstance {
    // キーを長い順にソートして、"UmamusumeScenario" が "Umamusume" より先にマッチするようにする
    const sortedEntries = Object.entries(returnValues)
      .sort(([a], [b]) => b.length - a.length);

    return jest.spyOn(service as never, 'loadJsonFile' as never)
  ...
  describe('onModuleInit', () => {
    it('3つの upsert メソッドを順番に呼び出す', async () => {
      ...
      expect(mockLogger.info).toHaveBeenCalledWith('マスタデータの差分チェックを開始します...');
      expect(mockLogger.info).toHaveBeenCalledWith('マスタデータの差分チェックが完了しました');
    });
  });
  ...
  describe('upsertRaces (via onModuleInit)', () => {
    it('DB に存在しない新規レースのみ createMany に渡す', async () => {
      ...
      // DB に「既存レース」が既に存在
      mockPrisma.raceTable.findMany.mockResolvedValue([{ race_name: '既存レース' }]);

      await service.onModuleInit();

      expect(mockPrisma.raceTable.createMany).toHaveBeenCalledTimes(1);
      const insertedData = mockPrisma.raceTable.createMany.mock.calls[0][0].data;
    ...
    it('全レースが既に存在する場合 → createMany を呼ばない', async () => {
      mockLoadJsonFile({
        Race: { '既存レース': { race_name: '既存レース', race_state: 0, distance: 1, distance_detail: null, num_fans: 0, race_rank: 1, senior_flag: false, classic_flag: false, junior_flag: false, race_months: 1, half_flag: false, larc_flag: false, bc_flag: false } },
        Umamusume: {},
        UmamusumeScenario: {},
      });
      mockPrisma.raceTable.findMany.mockResolvedValue([{ race_name: '既存レース' }]);
      ...
      expect(mockLogger.info).toHaveBeenCalledWith('race_table: 追加データなし');
    });
  });
    ...
    it('DB に存在しない新規ウマ娘のみ createMany に渡す', async () => {
      mockLoadJsonFile({
        ...
        Umamusume: {
          'ゴルシ': {
            umamusume_name: 'ゴルシ',
            turf_aptitude: 'A', dirt_aptitude: 'B',
            front_runner_aptitude: 'G', early_foot_aptitude: 'C',
            midfield_aptitude: 'A', closer_aptitude: 'A',
            sprint_aptitude: 'G', mile_aptitude: 'C',
            classic_aptitude: 'A', long_distance_aptitude: 'A',
          },
        },
        UmamusumeScenario: {},
      });
      ...
      const insertedData = mockPrisma.umamusumeTable.createMany.mock.calls[0][0].data;
  ...
  describe('upsertScenarioRaces (via onModuleInit)', () => {
    ...
    function setupScenarioMocks(
      ...
      mockPrisma.raceTable.findMany
        ...
        .mockResolvedValueOnce([   // upsertScenarioRaces: レースマスタ
      ...
      mockPrisma.umamusumeTable.findMany
        ...
        .mockResolvedValueOnce([   // upsertScenarioRaces: ウマ娘マスタ
    ...
    it('形式A（文字列）: レース名からレコードを生成する', async () => {
      ...
      const insertedData = mockPrisma.scenarioRaceTable.createMany.mock.calls[0][0].data;
    ...
    it('形式C/D（ネスト選択肢）: random_group が設定される', async () => {
      ...
      expect(insertedData[1]).toEqual(expect.objectContaining({
        race_id: 20,
        race_number: 1,
        random_group: 2,
      }));
    ...
    it('レース名がマスタに存在しない場合 → warn ログを出力してスキップする', async () => {
      ...
      mockPrisma.raceTable.findMany
        ...
        .mockResolvedValueOnce([   // upsertScenarioRaces: レースマスタ（該当なし）
          { race_id: 10, race_name: '日本ダービー' },
        ]);
      mockPrisma.umamusumeTable.findMany
    ...
    it('既にシナリオ登録済みのウマ娘 → スキップする', async () => {
      ...
      mockPrisma.umamusumeTable.findMany
        ...
        .mockResolvedValueOnce([{ umamusume_id: 1, umamusume_name: 'スペ' }]);
      // 既にシナリオ登録済み
      mockPrisma.scenarioRaceTable.findMany.mockResolvedValue([{ umamusume_id: 1 }]);
  ...
  describe('loadJsonFile エラーハンドリング', () => {
    it('ファイル読み込み失敗時に DatabaseException をスローする', async () => {
      jest.spyOn(service as never, 'loadJsonFile' as never)
        .mockRejectedValueOnce(
          new DatabaseException('シードデータの読み込みに失敗しました: data/Race.json', 'SeedService.upsertRaces', 'DB_002'),
        );
  ...
  describe('batchInsert エラーハンドリング', () => {
    it('DB 投入失敗時に DatabaseException をスローする', async () => {
      mockLoadJsonFile({
        Race: {
          '新レース': {
            race_name: '新レース', race_state: 0, distance: 1, distance_detail: null,
            num_fans: 0, race_rank: 1, senior_flag: false, classic_flag: false,
            junior_flag: false, race_months: 1, half_flag: false, larc_flag: false, bc_flag: false,
          },
        },
        Umamusume: {},
        UmamusumeScenario: {},
      ...
      mockPrisma.raceTable.createMany.mockRejectedValue(new Error('DB接続エラー'));
